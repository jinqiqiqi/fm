...
function get_openid($openid = NULL) {
    ...
    if (empty ( $openid ) && $isWeixinBrowser) {
        ...
        redirect(U('Error/Browser/index'));
    }
...
function getWeixinUserInfo($openid, $token) {
    ...
    if($content['errcode'] == 40001) {
        M('access_token') -> where("token = '%s'",$token) -> save(array("expire_time" => (time() - 1)));
        $access_token = get_access_token ( $token );
...
function get_access_token($token = '') {
    ...
    $at_map['token'] = $token;
    $at = M('access_token') -> where($at_map) -> find();
    if($at && $at['expire_time'] > time()) {
        ...
        $tempArr = json_decode ( httpGet ( $url ), true );        
        if($at) {           
            $data['expire_time'] = time() + 7000;
            $data['access_token'] = $tempArr['access_token'];
            M('access_token') -> where($at_map) -> save($data);
        } else {
            $data['token'] = $token;
            $data['expire_time'] = time() + 7000;
            $data['access_token'] = $tempArr['access_token'];
...
function get_access_token_qy($token = '') {
    empty ($token) && $token = get_token();
    $key = 'access_token_' . $token;
    $at_map['token'] = $token;
    $at = M('access_token') -> where($at_map) -> find();
